...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	[reference observeSingleEventOfType:FIRDataEventTypeValue withBlock:^(FIRDataSnapshot *snapshot)
	{
		...
		else if (block != nil) block([NSError description:@"Item not found." code:100]);
	}];
}
...
- (void)encodeWithCoder:(NSCoder *)encoder {
    ...
    [encoder encodeObject:self.subpath forKey:@"subpath"];
...
- (id)initWithCoder:(NSCoder *)decoder {
    if((self = [super init])) {
        ...
        self.dictionary = [decoder decodeObjectForKey:@"dictionary"];
    }
    return self;
}
@end
